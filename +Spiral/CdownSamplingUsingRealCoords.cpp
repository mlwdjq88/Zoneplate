...
double * downSampling(double * B, double lambda, double delta, double f,long & rowLen,double AccuCtrl){
    ...
    for (int i=0;i<rowLen;i++){
        Rs=pow(sqrt(pow(B[i],2)+pow(B[i+rowLen],2))+delta/2,2);
        K=K+(sqrt(Rs+pow(f,2))-f)/lambda-2;
    }
    K=K/rowLen;// ring numbers
    ...
    // remove the coordinates higher than the accuracy
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    for(int i=0; i< cellLen ; i++){
        ...
        rowLen          = mxGetM(B);
        pB = mxGetPr(B);
        C=downSampling(pB,lambda,delta,f,rowLen,AccuCtrl);// downsampling coordinates
