...
double getIntensity(double xs,double ys,double delta,double f,double lambda,double incidentAngle){
    ...
    //hologram function 
    I=pow(cos(2*PI/lambda*sqrt(pow(xs-delta/2,2)+pow(ys-offset,2)+pow(f,2))+atan2(ys,xs-delta/2))+cos(2*PI/lambda*sqrt(pow(xs+delta/2,2)+pow(ys-offset,2)+pow(f,2))-atan2(ys,xs+delta/2))+2*cos(2*PI/lambda*ys*sin(incidentAngle)),2)\
           +pow(-sin(2*PI/lambda*sqrt(pow(xs-delta/2,2)+pow(ys-offset,2)+pow(f,2))+atan2(ys,xs-delta/2))-sin(2*PI/lambda*sqrt(pow(xs+delta/2,2)+pow(ys-offset,2)+pow(f,2))-atan2(ys,xs+delta/2))+2*sin(2*PI/lambda*ys*sin(incidentAngle)),2);
