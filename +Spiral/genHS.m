...
Ts(3,:)=linspace(0.10,1.2,9);
Ts(4,:)=linspace(0.14,1.6,9);
Ts(5,:)=linspace(0.16,2,9);
Ts(1,10)=6;
Ts(2,10)=8;
Ts(3,10)=12;
Ts(4,10)=16;
...
filenamestr=[num2str(i),num2str(j),'wave',num2str(lambda),'_d',num2str(delta)];
filename=['C:\Users\T470P\Desktop\',filenamestr,'.gds'];
outputFile=OpenGDS(filename,filenamestr);
incidentAngle=incidentAngle/180*pi;
% R=3.8;
%  R=R/2;
% RingNum=(sqrt(f^2+R^2)-f)/lambda;
RingNum=(sqrt(f^2+(R+delta/2)^2)-f)/lambda-(sqrt(f^2+(delta/2)^2)-f)/lambda;
